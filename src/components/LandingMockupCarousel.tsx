...
const AUTO_SCROLL_INTERVAL = 4000; // ms - slightly longer for smoother experience
      ...
      <Carousel>
        <CarouselContent>
          {HERO_IMAGES.map((imgObj, i) => (
            <CarouselItem key={i} className={`transition-all duration-1000 ease-in-out ${i === activeIndex ? "opacity-100 scale-100" : "opacity-30 scale-95"}`}>
              <div className={`transition-opacity duration-500 ${isTransitioning ? 'opacity-70' : 'opacity-100'}`}>
                <img
                  src={imgObj.img}
                  alt={imgObj.text}
