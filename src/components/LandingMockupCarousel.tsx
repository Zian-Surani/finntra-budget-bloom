...
const LandingMockupCarousel: React.FC<CarouselProps> = ({
  ...
  return <div className="w-full max-w-2xl mx-auto relative mt-6">
      <div className="flex justify-center gap-4">
        <Carousel>
          <CarouselContent>
            {mockups.map((mockup, i) => <CarouselItem key={i} className={`flex justify-center transition-transform duration-500 ${i === activeIndex ? "scale-100 z-10" : "scale-90 opacity-60"}`} style={{
            transform: i === activeIndex ? "scale(1) translateY(0px)" : "scale(0.91) translateY(12px)"
                ...
                <div className="bg-white/90 dark:bg-gray-900/90 rounded-2xl shadow-xl overflow-hidden p-2 flex justify-center items-center h-72 w-52 md:w-64 border border-muted animate-scale-in transition-all hover:scale-105">
                  <img src={mockup.img} alt={mockup.alt} className="h-full w-full object-cover rounded-2xl shadow-md transition-shadow duration-300" loading="lazy" />
                </div>
              </CarouselItem>)}
          </CarouselContent>
      ...
      <div className="flex justify-center gap-1 mt-4">
        {mockups.map((_, idx) => <span key={idx} className={`h-2 w-4 rounded-full block transition-all duration-200 ${idx === activeIndex ? "bg-indigo-500" : "bg-gray-300 dark:bg-gray-700"}`} />)}
      </div>
